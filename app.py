...
from models import db, User, Conversion, get_database_url
...
def register_routes(app):
    ...
    def convert():
        ...
        try:
            ...
            original_path = os.path.join(app.config['UPLOAD_FOLDER'], f"{file_uuid}_{original_filename}")
            ...
            conversion = Conversion(
                user_id=session.get('user_id'), original_filename=original_filename,
                original_size=len(image_data), original_width=original_width,
                original_height=original_height, pixel_size=pixel_size,
                palette=palette, result_filename=result_filename,
                result_size=len(result_data), processing_time_ms=processing_time,
                status='completed')
            db.session.add(conversion)
            db.session.commit()
            return jsonify({
                'success': True, 'conversion_id': conversion.id,
                'result_filename': result_filename, 'processing_time_ms': processing_time,
                'original_size': len(image_data), 'result_size': len(result_data)
            })
    ...
    def history():
        ...
        conversions = Conversion.query.filter_by(user_id=user.id)\
    ...
    def api_stats():
        ...
        today_conversions = Conversion.query.filter(
            db.func.date(Conversion.created_at) == today).count()
        return jsonify({'total_conversions': total_conversions,
